...
- 프론트엔드 CI는 Node.js 22로 `npm ci`, `npm run test`, `npm run build`를 실행하며, `BACKEND_ORIGIN=http://127.0.0.1:8080`, `BASE_API_URL=http://127.0.0.1:8080/api/v1`을 빌드용 값으로 사용합니다.
- 백엔드 CI는 Java 17로 `./gradlew test`와 `./gradlew bootJar`를 실행합니다.
