...
public interface PostFavoriteControllerDocs {
    ...
    @Operation(summary = "내 즐겨찾기 목록 조회", description = """
    ...
    @SecurityRequirement(name = "bearerAuth")
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "즐겨찾기 상태 변경 성공", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
            {
              "code": "SUCCESS",
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "인증 토큰 없음"),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "게시글 없음 / soft delete / 비공개 게시글에 비작성자가 접근", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
            {
              "code": "POST_NOT_FOUND",
    ...
    ResponseEntity<ApiResponse<PostFavoriteResponse>> addFavorite(
        @Parameter(hidden = true) @AuthenticationPrincipal
        PrincipalDetails principalDetails,
        @Parameter(description = "즐겨찾기할 게시글 ID", example = "10") @PathVariable
        Long postId);

    @Operation(summary = "즐겨찾기 해제", description = """
    ...
    ResponseEntity<ApiResponse<PostFavoriteResponse>> removeFavorite(
        @Parameter(hidden = true) @AuthenticationPrincipal
        PrincipalDetails principalDetails,
        @Parameter(description = "즐겨찾기를 해제할 게시글 ID", example = "10") @PathVariable
        Long postId);
}
