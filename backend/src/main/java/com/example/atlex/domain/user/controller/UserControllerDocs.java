...
public interface UserControllerDocs {
    ...
    @ApiResponses({
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "입력값 오류 또는 약관 미동의",
            content = @Content(mediaType = "application/json",
                examples = @ExampleObject(value = """
                    {
    ...
    @ApiResponses({
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "타인의 계정 조회 시도",
            content = @Content(mediaType = "application/json",
                examples = @ExampleObject(value = """
                    {
                      "code": "ACCESS_DENIED",
    ...
    ResponseEntity<ApiResponse<Void>> checkEmail(
            @Parameter(description = "중복 확인할 이메일", example = "john@example.com") @RequestParam String email);

    @Operation(deprecated = true, summary = "[Deprecated] 이메일 중복 확인 - GET /api/v1/users?email={email} 사용 권장")
    ResponseEntity<ApiResponse<Void>> checkEmailDeprecated(@Valid @RequestBody EmailCheckRequest request);
    ...
    ResponseEntity<ApiResponse<Void>> changePassword(
            @Parameter(hidden = true) @AuthenticationPrincipal PrincipalDetails principalDetails,
            @Parameter(description = "비밀번호를 변경할 사용자 아이디", example = "john123") @PathVariable String userId,
    ...
    ResponseEntity<Void> delete(
            @Parameter(hidden = true) @AuthenticationPrincipal PrincipalDetails principalDetails,
            @Parameter(description = "탈퇴할 사용자 아이디", example = "john123") @PathVariable String userId);
}
