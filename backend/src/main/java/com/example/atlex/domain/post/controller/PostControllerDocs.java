...
public interface PostControllerDocs {
    ...
    @ApiResponses({
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "입력값 오류 (제목/내용 누락 또는 길이 초과)",
            content = @Content(mediaType = "application/json",
                examples = @ExampleObject(value = """
                    {
    ...
    ResponseEntity<ApiResponse<Page<PostSummaryResponse>>> getPostList(
            @Parameter(hidden = true) @AuthenticationPrincipal PrincipalDetails principalDetails,
            @Parameter(description = "정렬 타입 (latest: 최신순)", example = "latest")
                @RequestParam(defaultValue = "latest") String type,
            @Parameter(description = "작성자 아이디로 필터링 (선택)", example = "john123")
                @RequestParam(required = false) String userId,
            @Parameter(description = "카테고리 ID로 필터링 (선택)", example = "2")
                @RequestParam(required = false) Long categoryId,
            @ParameterObject Pageable pageable);

    @Operation(summary = "게시글 상세 조회",
        description = "게시글 ID로 상세 내용을 조회합니다. 비공개 게시글은 작성자 본인만 조회 가능합니다.")
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "조회 성공",
            content = @Content(mediaType = "application/json",
                examples = @ExampleObject(value = """
    ...
    ResponseEntity<ApiResponse<PostResponse>> getPost(
            @Parameter(hidden = true) @AuthenticationPrincipal PrincipalDetails principalDetails,
            @Parameter(description = "조회할 게시글 ID", example = "1") @PathVariable Long postId);
    ...
    @ApiResponses({
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "타인의 게시글 수정 시도",
            content = @Content(mediaType = "application/json",
                examples = @ExampleObject(value = """
                    {
                      "code": "ACCESS_DENIED",
    ...
    @ApiResponses({
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "타인의 게시글 삭제 시도",
            content = @Content(mediaType = "application/json",
                examples = @ExampleObject(value = """
                    {
                      "code": "ACCESS_DENIED",
    ...
    ResponseEntity<Void> deletePost(
            @Parameter(hidden = true) @AuthenticationPrincipal PrincipalDetails principalDetails,
            @Parameter(description = "삭제할 게시글 ID", example = "1") @PathVariable Long postId);
}
