...
public interface CategoryControllerDocs {
    ...
    @Operation(summary = "카테고리 목록 조회",
        description = "특정 사용자의 카테고리 목록을 커서 기반으로 조회합니다.")
    @ApiResponses({
    ...
    ResponseEntity<ApiResponse<CategoryListResponse>> getCategories(
            @Parameter(description = "카테고리를 조회할 사용자 아이디", example = "john123") @PathVariable String userId,
            @Parameter(description = "한 페이지 최대 항목 수 (기본값: 20)", example = "20") @RequestParam(required = false) Integer limit,
            @Parameter(description = "이전 응답의 nextCursor 값 (첫 페이지는 생략)", example = "5") @RequestParam(required = false) Long cursor,
    ...
    ResponseEntity<ApiResponse<CategoryResponse>> createCategory(
            @Parameter(description = "카테고리를 생성할 사용자 아이디", example = "john123") @PathVariable String userId,
    ...
    @ApiResponses({
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "타인의 카테고리 수정 시도",
            content = @Content(mediaType = "application/json",
                examples = @ExampleObject(value = """
                    {
                      "code": "ACCESS_DENIED",
    ...
    ResponseEntity<ApiResponse<CategoryResponse>> updateCategory(
            @Parameter(description = "카테고리 소유자 아이디", example = "john123") @PathVariable String userId,
            @Parameter(description = "수정할 카테고리 ID", example = "1") @PathVariable Long categoryId,
    ...
    @ApiResponses({
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "타인의 카테고리 삭제 시도",
            content = @Content(mediaType = "application/json",
                examples = @ExampleObject(value = """
                    {
                      "code": "ACCESS_DENIED",
    ...
    ResponseEntity<Void> deleteCategory(
            @Parameter(description = "카테고리 소유자 아이디", example = "john123") @PathVariable String userId,
            @Parameter(description = "삭제할 카테고리 ID", example = "1") @PathVariable Long categoryId,
            @Parameter(hidden = true) @AuthenticationPrincipal PrincipalDetails principalDetails);
}
