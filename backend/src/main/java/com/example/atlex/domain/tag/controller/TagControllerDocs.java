...
public interface TagControllerDocs {
    ...
    @Operation(summary = "태그 목록 조회",
        description = "특정 사용자의 태그 목록을 커서 기반으로 조회합니다.")
    @ApiResponses({
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "userId 누락 또는 limit/cursor 값 오류",
            content = @Content(mediaType = "application/json",
                examples = @ExampleObject(value = """
                    {
    ...
    ResponseEntity<ApiResponse<TagListResponse>> getTags(
            @Parameter(description = "태그를 조회할 사용자 아이디", example = "john123") @RequestParam String userId,
            @Parameter(description = "한 페이지 최대 항목 수", example = "20") @RequestParam(required = false) Integer limit,
