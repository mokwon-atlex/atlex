...
public interface CommentControllerDocs {
    ...
    ResponseEntity<ApiResponse<CommentResponse>> createComment(
        ...
        @Parameter(description = "댓글을 작성할 게시글 ID", example = "10") @PathVariable
        Long postId,
        @Valid @RequestBody
    ...
    ResponseEntity<ApiResponse<List<CommentResponse>>> getComments(
        @Parameter(hidden = true) @AuthenticationPrincipal
        PrincipalDetails principalDetails,
        @Parameter(description = "댓글을 조회할 게시글 ID", example = "10") @PathVariable
        Long postId);
    ...
    @ApiResponses({
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "인증 토큰 없음"),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "타인의 댓글 수정 시도", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
            {
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "댓글 없음 (또는 삭제된 게시글의 댓글)", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
            {
    ...
    ResponseEntity<ApiResponse<CommentResponse>> updateComment(
        @Parameter(hidden = true) @AuthenticationPrincipal
        PrincipalDetails principalDetails,
        @Parameter(description = "수정할 댓글 ID", example = "1") @PathVariable
        Long commentId,
        @Valid @RequestBody
    ...
    @ApiResponses({
        ...
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "인증 토큰 없음"),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "403", description = "댓글 작성자/게시글 작성자가 아닌 사용자의 삭제 시도", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
            {
    ...
    ResponseEntity<Void> deleteComment(
        @Parameter(hidden = true) @AuthenticationPrincipal
        PrincipalDetails principalDetails,
        @Parameter(description = "삭제할 댓글 ID", example = "1") @PathVariable
        Long commentId);
}
