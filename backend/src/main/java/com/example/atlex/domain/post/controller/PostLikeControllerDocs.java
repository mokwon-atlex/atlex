...
public interface PostLikeControllerDocs {
    ...
    @Operation(summary = "좋아요 등록", description = """
    ...
    @ApiResponses({
        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "좋아요 상태 변경 성공", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
            {
              "code": "SUCCESS",
    ...
    ResponseEntity<ApiResponse<PostLikeResponse>> like(
        ...
        @Parameter(description = "좋아요할 게시글 ID", example = "10") @PathVariable
        Long postId);
    ...
    ResponseEntity<ApiResponse<PostLikeResponse>> unlike(
        @Parameter(hidden = true) @AuthenticationPrincipal
        PrincipalDetails principalDetails,
        @Parameter(description = "좋아요를 취소할 게시글 ID", example = "10") @PathVariable
        Long postId);
}
