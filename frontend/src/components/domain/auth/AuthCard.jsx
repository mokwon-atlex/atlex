  ...
  return (
    <section className="flex items-center justify-center py-6">
      <div className="w-full rounded-3xl border border-border/60 bg-background p-6 lg:p-8">
        ...
        {mode === 'login' && <LoginForm onSwitchMode={() => onModeChange('terms')} />}

        {mode === 'terms' && <TermsStep onNext={() => onModeChange('signup')} onBack={() => onModeChange('login')} />}

        {mode === 'signup' && <SignupForm onBack={() => onModeChange('terms')} onLogin={() => onModeChange('login')} />}
      </div>
    </section>
  );
}
