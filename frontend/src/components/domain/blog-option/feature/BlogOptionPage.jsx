...
import {
  createUserBlogCategory,
  deleteUserBlogCategory,
  updateUserBlogCategory,
} from "@/lib/api/users";
...
import {
  BLOG_OPTION_BIO_MAX_LENGTH,
  normalizeBlogBio,
} from "@/lib/blog-option";
...
export default function BlogOptionPage() {
  ...
  useEffect(() => {
    ...
    async function loadBlogOption() {
      ...
      try {
        ...
        const message =
          error?.message ?? "블로그 옵션 정보를 불러오지 못했습니다.";
        setBioError(message);
  ...
  async function handleUpdateCategory(event, categoryId) {
    ...
    const { name: nextCategoryName, error } = validateCategoryName(
      editingCategoryName,
      categoryId
    );
    ...
    <BlogOptionPageLayout
      ...
      bioSection={
        <BlogOptionBioForm
          ...
          onBioChange={(value) =>
            setBio(value.slice(0, BLOG_OPTION_BIO_MAX_LENGTH))
          }
          onReset={() => {
            setBio(savedBio);
            setBioError("");
