  ...
  return (
    <Card className="rounded-[2rem] border-border/60 bg-card/80 shadow-sm backdrop-blur">
      <CardHeader>
        ...
        <CardDescription>블로그 프로필 카드에 노출되는 소개글을 작성합니다.</CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        {!isLoading && !bio && !savedBio ? (
          <p className="text-sm text-muted-foreground">아직 저장된 소개글이 없습니다.</p>
        ) : null}
      </CardContent>

      <CardFooter className="justify-end gap-3">
        <Button type="button" variant="outline" disabled={isLoading || isSaving} onClick={onReset}>
          되돌리기
        </Button>
