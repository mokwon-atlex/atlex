...
export default function PostEditorToolPopover({ getItemState, onExecuteItem, tool }) {
  ...
  return (
    <Popover>
      <PopoverTrigger
        render={
          <Button type="button" variant="outline" size="icon-sm" aria-label={`${tool.title} 도구 열기`}>
      ...
      <PopoverContent side="top" sideOffset={8} className="w-auto gap-0 p-3">
        {tool.groups.map((group, index) => (
          <div key={group.id}>
            ...
            <p className="mb-1.5 px-0.5 text-xs font-semibold text-muted-foreground">{group.title}</p>
