...
const COLOR_GROUP_NOTICE = '색상 관련 기능은 이번 작업 범위에서 제외해서 버튼만 비활성화해 두었습니다.';
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex items-center justify-between border-b border-border px-5 py-4">
        <div>
          <Textfield variant="muted" size="xs" weight="semibold" className="uppercase tracking-[0.18em]">
            도구 패널
          </Textfield>
        ...
        <Button type="button" variant="outline" size="icon-sm" onClick={onClose} aria-label="도구 패널 닫기">
          <X className="h-4 w-4" />
        </Button>
      </div>
        ...
        <div className="space-y-5 p-5">
          <Card size="sm" className="bg-muted/40">
            <CardHeader>
              ...
              <CardDescription className="leading-6">{activeTool.caption}</CardDescription>
            </CardHeader>
          </Card>
          ...
          <section>
            <div>
              ...
              <Textfield variant="muted" size="sm" className="mt-2 leading-6">
                도구 묶음 버튼을 누르면 해당 영역이 열리고 거기서 다시 세부 그룹을 선택할 수 있습니다.
              </Textfield>
            </div>
            ...
            <div className="mt-4 rounded-xl border border-border bg-muted/40 p-4">
              <div className="border-b border-border pb-4">
                ...
                {activeGroup.id === 'font-color' ? (
                  <Alert variant="warning" className="mt-3">
                    <AlertDescription className="leading-6">{COLOR_GROUP_NOTICE}</AlertDescription>
                  </Alert>
                ) : null}
              </div>
              ...
              <div className="mt-4 grid gap-3">
                {activeGroup.items.map((item) => {
                  const { isActive, isDisabled } = getItemState(activeGroup.id, item);
