...
export default function BlogHomeTagChip({ active = false, count, fullWidth = false, label, onClick }) {
  return (
    <Button
      onClick={onClick}
      type="button"
        ...
        {typeof count === 'number' ? (
          <span
            className={cn(
              ...
              active ? 'bg-primary-foreground/14 text-primary-foreground' : 'bg-muted text-muted-foreground',
            )}
          >
            {count}
          </span>
