  ...
  render: () => {
    ...
    return (
      <div className="w-[200px] rounded-xl border border-border bg-card p-4">
        <h3 className="mb-4 text-xs font-bold uppercase tracking-wider text-muted-foreground">메뉴</h3>
        <ul className="space-y-1">
          {items.map((item) => (
            <NavItem key={item.id} isActive={activeId === item.id} onClick={() => setActiveId(item.id)}>
              {item.title}
            </NavItem>
          ))}
        </ul>
